...
def scrape_verbix(verb):
    ...
    if len(present_table["ich"].split(" ")) > 1:
        verb_conj['prefix'] = present_table["ich"].split(" ")[1]

    """ Parsing present tense """
    for pronoun in present_table:
        present_table[pronoun] = present_table[pronoun].split(" ")[0] # stripping prefix
    ...
    [aux_verb, pp] = perfect_table['wir'].split(" ")
    ...
    verb_conj['imperfect'] = imperfect_table['ich'].split(" ")[0]

    """ Parsing imperfect tense """

    konjunctiv_table = get_conjugation_table(soup, "Conjunctive I and II", "Past")

    verb_conj['konjunctiv'] = konjunctiv_table['ich'].split(" ")[0]
